...
export default function HomeView() {
  const { leaderboard, onlinePlayers, setCurrentView, initGame, isAuthenticated, handleLogoClick, challenges } = useGameStore();
    ...
    <div className="min-h-screen bg-[#0a0a0f] relative overflow-hidden pb-24">
      ...
      <div className="relative z-10 text-center pt-10 pb-6 px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <span>•</span>
          <span>⚔️ {challenges.length} défis actifs</span>
        </motion.div>
      ...
      <div className="px-4 space-y-5 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1 }}>
          ...
          <div className="space-y-2">
            {[
              { p1: 'Kofi M.', p2: 'Amara D.', bet: 25000, currency: 'CDF', time: '12min' },
              { p1: 'Fatou C.', p2: 'Kwame A.', bet: 9, currency: 'USD', time: '3min' },
              { p1: 'Aisha T.', p2: 'Boubacar B.', bet: 50000, currency: 'CDF', time: '28min' },
            ].map((m, i) => (
              <div key={i} className="glass rounded-xl px-4 py-3 border border-white/5 flex items-center gap-3">
                <div className="flex-1 flex items-center gap-2">
                  <span className="text-white text-sm font-medium">{m.p1}</span>
                  <span className="text-white/30 text-xs">⚔️</span>
                  <span className="text-white text-sm font-medium">{m.p2}</span>
                </div>
                <div className="text-right">
                  <p className="text-yellow-400 text-xs font-bold">{m.currency === 'USD' ? `$${m.bet}` : `${m.bet.toLocaleString()} CDF`}</p>
                  <p className="text-white/30 text-xs">⏱ {m.time}</p>
                </div>
