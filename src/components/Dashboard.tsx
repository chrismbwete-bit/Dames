...
import { Trophy, TrendingUp, Target, Zap, Users, Star, ChevronRight } from 'lucide-react';
import { useGameStore, AIDifficulty } from '../store/gameStore';

const AI_LEVELS: { diff: AIDifficulty; label: string; color: string; bg: string }[] = [
  { diff: 'simple', label: '🟢 Simple', color: 'text-green-400', bg: 'bg-green-500/20 border-green-500/30 hover:bg-green-500/30' },
  { diff: 'easy', label: '🔵 Facile', color: 'text-blue-400', bg: 'bg-blue-500/20 border-blue-500/30 hover:bg-blue-500/30' },
  { diff: 'normal', label: '🟡 Normal', color: 'text-yellow-400', bg: 'bg-yellow-500/20 border-yellow-500/30 hover:bg-yellow-500/30' },
  { diff: 'hard', label: '🔴 Difficile', color: 'text-red-400', bg: 'bg-red-500/20 border-red-500/30 hover:bg-red-500/30' },
  { diff: 'extreme', label: '💀 Dur', color: 'text-purple-400', bg: 'bg-purple-500/20 border-purple-500/30 hover:bg-purple-500/30' },
];

export default function Dashboard() {
  const {
    currentUser, leaderboard, challenges, setCurrentView,
    initGame, acceptChallenge, declineChallenge, transactions,
    setAIDifficulty, adminSettings,
  } = useGameStore();
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-white">Partie Gratuite vs IA</h3>
            </div>
            <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full border border-green-500/30">GRATUIT</span>
          </div>
          <p className="text-white/50 text-sm mb-4">Entraînez-vous sans mise — 5 niveaux d'intelligence</p>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {AI_LEVELS.map(({ diff, label, bg }) => (
              <motion.button key={diff} whileTap={{ scale: 0.95 }}
                onClick={() => {
                  setAIDifficulty(diff);
                  initGame('ai', { aiDiff: diff });
                }}
                className={`py-2.5 rounded-xl text-xs font-bold transition-all border ${bg}`}>
                {label}
              </motion.button>
            ))}
          </div>
