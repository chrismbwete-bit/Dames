              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div className="ml-auto">
                  <span className={`px-2 py-1 rounded-lg text-xs font-bold ${
                    viewChallenge.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400' :
                    viewChallenge.status === 'accepted' ? 'bg-green-500/20 text-green-400' :
                    'bg-red-500/20 text-red-400'
                  }`}>
                    {viewChallenge.status === 'pending' ? '⏳ En attente' :
                     viewChallenge.status === 'accepted' ? '✅ Accepté' : '❌ Refusé'}
                  </span>
                </div>
              </div>
              ...
              {viewChallenge.status === 'accepted' && (
                <motion.button whileTap={{ scale: 0.97 }}
                  onClick={() => {
                    ...
                    initGame('challenge', {
                      betAmount: viewChallenge.betAmount,
                      currency: viewChallenge.currency as 'CDF' | 'USD',
                      pieceCount: viewChallenge.pieceCount,
                      boardSize: viewChallenge.boardSize,
                      timePerTurn: viewChallenge.timePerTurn,
                      opponentName: viewChallenge.fromPlayerName,
                    });
                  }}
                  className="w-full mt-4 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl font-bold flex items-center justify-center gap-2">
                  <Play size={16} /> Observer le match
                </motion.button>
              )}
      ...
      <div className="p-4 bg-gradient-to-b from-red-900/30 to-transparent border-b border-red-500/20 sticky top-0 z-10 backdrop-blur-xl">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {[
            ...
            { label: 'Défis ⏳', value: pendingChallenges.length, icon: '⚔️', color: 'text-yellow-400' },
      ...
      <div className="p-4 space-y-4">
        ...
        {activeTab === 'challenges' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'En attente', value: pendingChallenges.length, color: 'text-yellow-400', bg: 'border-yellow-500/20', icon: <AlertCircle size={16} className="text-yellow-400" /> },
            ...
            {(['pending', 'accepted', 'declined'] as const).map(status => {
              ...
              const statusInfo = {
                ...
                declined: { label: '❌ Refusés', color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/20' },
              }[status];
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Défis en attente', value: pendingChallenges.length, color: 'text-red-400', icon: '⚔️' },
